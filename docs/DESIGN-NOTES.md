...
- `chrome.ts` keeps `--remote-debugging-pipe` on Windows: proven on a `windows-latest` runner on 10 Sep 2026, where `Browser.getVersion` answered over fd 3/4 under Bun's spawn exactly as it does on macOS. The WebSocket fallback (`--remote-debugging-port=0` and `DevToolsActivePort`) answered too and is the thing to reach for if that ever stops, but a second transport would be two code paths bought for nothing today. `scripts/windows-cdp-spike.ts` re-asks the question by hand on a Windows machine; nothing runs it on a schedule, because `ci.yml` now runs the suites there and they cover the same ground with more of the product behind them.
- `serveStatic` normalises the request path with `path.posix.normalize`, not `path.normalize`: a URL path is always forward slashes, and on Windows the native call turns `/` into `\\`, so `isAppShellPath` stopped recognising the app shell and every phone got a 404 for the page itself. The traversal guard is unchanged — `path.join` still builds the filesystem path and still has to land inside `WEB_DIST`.
- `smoke.mjs` skips the resize-notification section on Windows rather than letting it pass: the test arms a `trap … WINCH` in the shell, PowerShell has no such signal, and the two checks asserting the trap did *not* fire would then pass for the wrong reason while the two asserting it did would fail.
- A screenshot's filename carries no colon, though a capture of `127.0.0.1:3099` obviously wants one. NTFS reads `name:stream` as an alternate data stream, so the PNG was written *into* a stream hanging off a file called `1789044528288-url-127.0.0.1`, and nothing afterwards could copy or serve it. The label is recovered from the filename, so it is sanitised too: a capture labelled `localhost:3000` is filed and shown as `localhost_3000`.
- Chrome gets sixty seconds to answer on Windows and twenty on macOS. The first launch on a Windows runner is the slow one — a fresh profile, and Defender reading a browser it has not seen — and it is the only capture in a suite run that ever failed. Every capture after it in the same process reuses the browser and returns in under a second, so the number is about a cold start, not about how long a hung browser should be waited for.
- `preview.ts` starts the Tailscale CLI through `platform.runsExecutableOnPath`, which is `cmd.exe /d /s /c` on Windows, for the same reason `setup.ts` reaches for `claude` that way: `execFile` there starts a `.exe` and nothing else, and the CLI a machine actually has may be a `.cmd`. It also lets `test.ts` hand `ORBIT_TAILSCALE` a one-line `.cmd` shim over `fake-tailscale.mjs`. Compiling that stand-in into a Bun executable instead looked tidier and was not: a Bun process spawning a Bun-compiled binary on Windows ran it back through `bun.exe` as though the `.exe` were source, and the parse errors that produced pointed nowhere near the cause. Left as a plain script with no shim at all, every Tailscale test there reported "not installed" and passed by skipping, which is the worst way for a test to be green.
- `Get-FileHash` is not on a `windows-latest` runner, though `Unblock-File` and `Invoke-WebRequest` from the same module are, so `install.ps1` hashes through `System.Security.Cryptography` and deletes the `Zone.Identifier` stream itself. Neither needs a module loaded, which is one fewer thing to be missing on a machine nobody here can inspect.
- `install.ps1` is ASCII only, and deliberately. PowerShell 5.1 — still the `powershell.exe` on every Windows box — reads a script with no byte-order mark as Windows-1252, so a UTF-8 em dash arrives as `\u00e2\u20ac\u201d`, and that last character is a right double quote, which PowerShell accepts as a string delimiter. One dash in a message ended the string early and the parse error landed six lines later on an unexpected `}`. `ci.yml` parses the file as a step of its own because the error PowerShell hands a caller says neither the line nor the reason.
- `home.userHome()` is the one place that decides where the user's home is, and every module asks it rather than `os.homedir()`. The suites used to isolate themselves by moving `HOME`, which works on macOS and does nothing on Windows, where `os.homedir()` reads `USERPROFILE` — and moving *that* stops Chrome starting at all: on a `windows-latest` runner on 10 Sep 2026, `launch()` captured happily twice and then timed out on its DevTools pipe with nothing changed but `USERPROFILE` pointing at a scratch directory. `ORBIT_HOME` is the name that works on both, so a suite wanting a home of its own sets that, not `HOME` alone.
