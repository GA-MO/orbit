...
- **↻ Resume** — ให้ agent **สานบทสนทนาเดิมต่อ** ในโฟลเดอร์นั้น (Claude Code ใช้
  `claude --continue`, Codex ใช้ `codex resume --last`) ปุ่มนี้จะขึ้นเฉพาะ agent
  ที่ทำได้ — Shell กับ Gemini ไม่มี ถ้าโฟลเดอร์นั้นไม่เคยคุยกันมาก่อน agent จะ
  บอกเองใน terminal ว่าไม่มีบทสนทนาให้ต่อ
