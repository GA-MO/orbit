...
function useSpeechSessionUpdates(session: SpeechSession) {
...
function statusLabel(preparing: boolean, listening: boolean) {
  if (preparing) return 'Waiting for microphone…'
  if (listening) return 'Listening…'
  return 'Stopped'
}

function LanguagePills({ session }: { session: SpeechSession }) {
  return (
...
export default function VoiceSheet({ session, onImage, onInsert, onSend, onClose }: Props) {
  useSpeechSessionUpdates(session)
    ...
    <Sheet title="Voice input" onClose={onClose}>
      <div className="flex flex-col gap-3 px-5 pt-1 pb-4">
        <div className="flex items-center gap-2 text-[13px]">
          ...
          <span className={live ? 'text-fore' : 'text-mut'}>{statusLabel(preparing, listening)}</span>
          <LanguagePills session={session} />
        </div>
        <textarea
