...
| ไฟล์ | เรื่อง |
| --- | --- |
| `web/src/views/CapturesView.tsx` | ปุ่ม Share → 44px |
...
`scripts/touch-smoke.mjs` ผ่าน **36/36** (อินสแตนซ์แยก พอร์ต 3099, `HOME` แยก)

**ที่ยังเหลือจากรอบก่อนหน้า** — ปุ่ม 📷 กับ ✕ ใน *แถว preview* เป็น `size-8` = 32px
ยังต่ำกว่า 44 เหมือนกัน และพลาด ✕ จะไปโดนพื้นที่ "เปิดเฟรม" ซึ่งแย่กว่าไม่เกิดอะไร
ยังไม่ได้แก้เพราะยังไม่ได้สั่ง
