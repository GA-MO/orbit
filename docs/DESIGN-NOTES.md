...
- `--remote-debugging-port=0` and the endpoint scraped from stderr, rather than a port picked here: a fixed port collides with the user's own Chrome and a probe-then-bind races it. `DEVTOOLS_ENDPOINT` matches the line Chrome prints when it is ready to be spoken to, so the wait for the endpoint *is* the wait for startup.
