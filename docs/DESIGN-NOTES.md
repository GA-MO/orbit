...
- `GALLERY_ENTRY_KEPT_OUT_OF_DIST` (`web/vite.config.ts`) is a build input only under `ORBIT_GALLERY=1`. The server hands out any file that exists in `dist` and redirects only a *miss* to `index.html`, so a built `dev.html` would be a live, unauthenticated page on the tailnet — the one thing this page must not become. Vite serves any root `.html` in dev anyway, so the gallery needs no build input to be usable; the flagged one exists so the page can be proven to compile. Building it always and deleting the file afterwards was rejected: "and then we remember to delete it" is not a guarantee, and the chunks it pulled in would still be sitting in `dist/assets`.
- `resolvedAgainstThisFile` resolves rollup inputs against `vite.config.ts` rather than leaving bare names: an input string is relative to the working directory, so `vite build web` from the repo root would look for `index.html` one directory too high.
- `EVERY_TAILNET_HOST` (`.ts.net`) is in `allowedHosts` because Vite checks the `Host` header and answers anything it does not recognise with a block page instead of the app. `tailscale serve` reaches the dev server under the tailnet name, so without it the whole point of publishing a port — tap the row, look at the app in a frame — shows Vite's refusal, and a capture photographs that refusal. A leading dot matches the host and its subdomains, so this is every tailnet without naming this machine's, which changes with the tailnet. Dev only: `vite build` never reads it.
...
### The command-line look (ui.ts, install.sh)

- Every command draws through one module rather than each printing its own lines. Before it, `setup`, `doctor`, `update` and `stop` each had a private `say()` and a private idea of what a good line looked like, so the same fact — a file written, a check that passed — appeared three ways in three commands. `ui.ts` owns the palette, the wordmark, the rules and the channel board; a command supplies facts.
- `interactive()` is `isTTY && !CI`, and `coloured()` is that plus `NO_COLOR`. Everything animated hangs off the first: a spinner, a progress bar and a repainted block are worth a great deal to someone watching and are noise in a pipe. So a piped run prints each channel once, in channel order, with no escape sequence in it at all — which is also why the smoke suites can keep asserting on plain substrings.
- `FORCE_COLOR` overrides both, and `FORCE_COLOR=3` forces truecolor. It is there for `make shots` and for looking at the output through a pipe while working on it; without it there is no way to see what the terminal sees.
- The palette is the app's, token for token, and not a terminal palette of its own. `PEARL` is `--pearl` — the primary button's face, the one object in the app that carries both light and colour — and it is what the wordmark, the rules and the `◍` are painted with. `ACCENT` is `--color-accent`, `ADDED` is `--color-add`, `DANGER` is `--color-danger`, `LIVE` is `--color-live`. The first pass invented a neon cyan-to-violet ramp instead, which looked like a different product: the app is deliberately near-monochrome, with two hues that mean something and silver for the rest, and a CLI that opens with neon undoes that in one screen.
- Only what carries meaning gets a hue, exactly as in the app: green for a check that passed, red for one that failed, amber for the one that is still running, and silver-dim for everything else. That is also why the spinner is `LIVE` rather than the accent — a spinner is the terminal's version of the app's live badge.
- The gradient is per-character interpolation along `PEARL`'s three stops (`alongRamp`), and it degrades rather than disappears: a terminal that does not announce `COLORTERM=truecolor` gets the whole run in the nearest of five basic colours (`nearestBasic`), not a 24-bit sequence it will print as text. 256-colour cubes were tried and dropped — the extra fidelity is invisible on a rule and a wordmark, and it doubled the palette code.
- `Board` repaints by moving the cursor up exactly as many lines as it last printed. That is only correct if no line wrapped, so `line()` and `fixLine()` truncate to `columns() - 1`: at exactly `columns()` some terminals wrap and some do not, and the one that wraps leaves the block drifting down the screen a line per frame.
- The spinner only repaints while some channel is `running`. A board of settled channels would otherwise redraw ten times a second to change nothing, which is what makes a terminal fan audible.
- The board is opened before the work starts, not after, so a slow check shows a spinner rather than a blank screen. `doctor` runs all nine checks concurrently and settles each as it lands — the `claude mcp get orbit` call alone can take fifteen seconds, and serially the whole report waited on it.
- `fix` renders as `└→` under the line it belongs to and is used for two different things on purpose: what to do about a failed check (`doctor`) and what a step did in passing (`setup` backing a file up). Both are a subordinate fact about the line above, and a second glyph for the second kind read as a second failure.
- `orbit update` streams the download (`drain`) instead of `arrayBuffer()`, purely so the progress bar is real. It falls back to `arrayBuffer()` when there is no callback or no body, and a release served without `content-length` shows a spinner rather than a bar that lies.
- `install.sh` repeats the look in bash rather than sharing it, because it runs before there is an executable to share anything with. Its steps print on completion with `\r` and `\033[K` over the "working" line — bash has no repaint loop, and the erase matters because a finished line is not always longer than the line it replaces.
- `install.sh` never prints the word `PATH` on the happy path. `install-smoke.mjs` asserts that a directory already on the PATH gets no advice by looking for that word in the output, so a heading or a hint that mentioned it would fail a test that is really about not nagging.
- `blockWordmark` draws ORBIT as half-block pixels and only `banner()` uses it, because the banner is the one screen that stays up. A mark that size on `doctor` or `stop` — commands you run and scroll past — is noise wearing a logo. Every other command gets the inline `O R B I T`.
- The letters are a 5×5 grid folded two pixel rows to a text row with `▀▄█`, then doubled horizontally so the pixels come out square in a terminal cell (which is about twice as tall as it is wide). That puts the mark at `BLOCK_WORDMARK_WIDTH` = 44 columns; below 48 the banner falls back to the inline wordmark rather than wrapping the mark into rubble.
- The gradient runs across the mark's columns, not per letter, so the ramp reads as one object.
- The mark goes on screens you *arrive at* and not on screens you *pass through*: `install.sh` (the first time anyone meets Orbit), `orbit help` (the CLI's front page) and the running server's banner. `doctor`, `setup`, `update`, `stop` and `pair` are transactions — you read a result and scroll on — and a logo on each of them is noise wearing a logo.
- `install.sh` builds the mark from the same `1`/`0` grid rather than storing the drawn `▀▄█` art, because bash substring arithmetic on multibyte glyphs depends on the locale and would slice a block character in half on a machine that is not UTF-8.
- `scripts/wordmark.mjs` (`make wordmark`) draws `docs/images/wordmark.svg` for the README from `stitchLetters()`, so the README and the terminal cannot drift apart. The gradient needs `gradientUnits="userSpaceOnUse"`: the default gives every rect its own full ramp, so the mark comes out as 55 individually rainbow-tinted squares rather than one object under one ramp.
- The README's SVG sits on a `#171614` card (`--color-surface`, `--radius-card`) rather than on the page. Pearl is a light gradient — it is a lit control on a dark app — and dropped straight onto GitHub's light theme the cream stop lands near-white on white and the middle of the mark disappears. The card is also what the button has behind it in the app, so this is the mark in its own habitat rather than a fix.
