...
- **What the framed page can do besides sit there**: a dev server changes under you, so the frame has **↻** — the page is a different origin, so there is no `contentWindow.location.reload()` to call and re-assigning the same `src` is not reliably a navigation; the iframe is remounted instead, which always fetches. Beside it, two ways to hand what you are looking at to the agent, because neither covers the other: **⧉** renders the whole page headless from the Mac at this phone's width, which gets everything below the fold but *cannot* carry the state on screen — a cross-origin frame will not report its scroll position, let alone an open menu, and the shot is the URL fetched again from nothing. For that there is **📷**, which is the phone's own screenshot: side button + volume up while the frame is open catches the scroll, the open modal, the half-filled form, in real WebKit rather than a Chrome re-render — the difference that decides whether an iOS-only layout bug is in the picture at all. Either way the path lands in the prompt and the frame closes, because the next thing to do is describe it
