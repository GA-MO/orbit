...
- `test.ts` compiles `fake-tailscale.mjs` into an executable on Windows before pointing `ORBIT_TAILSCALE` at it. `preview.ts` runs the CLI with `execFile`, which on Windows can start a `.exe` and nothing else — not a `.mjs` with a shebang, and not a `.cmd` shim either, since Node refuses those without a shell. Left as a script, every Tailscale test there reported "not installed" and passed by skipping, which is the worst way for a test to be green.
